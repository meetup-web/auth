...
from auth.application.operations.write.login import UserLoggedIn
...
class CreateSessionOnUserLoggedInHandler(NotificationHandler[UserLoggedIn]):
    def __init__(
        self,
        session_factory: SessionFactory,
        session_repository: SessionRepository,
    ...
    async def handle(self, notification: UserLoggedIn) -> None:
        session = self._session_factory.create_session(user_id=notification.user_id)
